...
#elif defined __AVR_ATmega644P__  // the ATmega644P has two USARTs, but we use the first
#define BL_DDR  DDRD
...
int main(void)
{
	...
	*/
	...
	UBRR0L = (uint8_t)((F_CPU/(BAUD_RATE*16L))-1);
	UBRR0H = ((F_CPU/(BAUD_RATE*16L))-1) >> 8;
	UCSR0B = (1<<RXEN0) | (1<<TXEN0);			// enable Tx/Rx of first USART
	UCSR0C = (3<<UCSZ00);						// set protocol to 8N1
	BL_DDR &= ~_BV(BL0);						// suppress noise
	BL_PORT |= _BV(BL0);
#else
...
#elif defined(__AVR_ATmega168__)  || defined(__AVR_ATmega328P__) || defined(__AVR_ATmega644P)
		getch();
#else
		/* m8,16,32,169,8515,8535,163 */
		/* 20060803 DojoCorp:: Addon coming from the previous Bootloader*/               
